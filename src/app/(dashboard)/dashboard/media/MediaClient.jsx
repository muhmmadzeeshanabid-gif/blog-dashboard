...
function CustomSingleValue(props) {
  const isDate = props.selectProps.inputId === "media-date-filter";
  let labelText = props.data.label.split(" (")[0];
  if (isDate && props.data.value === "all") {
    labelText = "Filters";
  }
  ...
  return (
    <components.SingleValue {...props}>
      <span style={{ display: "inline-flex", alignItems: "center", gap: "8px", color: "inherit" }}>
        {isDate && (
          <i className="fas fa-sliders-h" style={{ fontSize: "13px", color: "var(--dashboard-icon)" }}></i>
        )}
        <span>{labelText}</span>
      </span>
...
function CustomOption(props) {
  const labelText = props.data.label.split(" (")[0];
  const count = props.data.label.match(/\((\d+)\)/)?.[1] || "";
...
export default function MediaClient({ initialMedia, isDarkInitial }) {
  ...
  const uniqueFolders = useMemo(() => {
    ...
    initialMedia.items.forEach((asset) => {
      const cat = asset.category || "Uncategorized";
      if (cat.trim() !== "") {
        folders.add(cat.trim());
      }
    });
  ...
  const filterTotals = useMemo(() => {
    const items = initialMedia.items;
    const now = Date.now();
    const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
    const oneMonthAgo = now - 30 * 24 * 60 * 60 * 1000;

    const dateWeek = items.filter((a) => {
      const assetTime = a.updatedAt ? new Date(a.updatedAt).getTime() : 0;
      return assetTime >= oneWeekAgo;
    }).length;
    const dateMonth = items.filter((a) => {
      const assetTime = a.updatedAt ? new Date(a.updatedAt).getTime() : 0;
      return assetTime >= oneMonthAgo;
    }).length;
    const dateOlder = items.filter((a) => {
      const assetTime = a.updatedAt ? new Date(a.updatedAt).getTime() : 0;
      return assetTime < oneMonthAgo;
    }).length;
  ...
  const filteredAssets = useMemo(() => {
    ...
    return initialMedia.items.filter((asset) => {
      ...
      const cat = asset.category || "Uncategorized";
      const matchFolder = activeFolder === "all" || cat === activeFolder;
      if (!matchFolder) return false;

      if (dateFilter !== "all") {
        const assetTime = asset.updatedAt ? new Date(asset.updatedAt).getTime() : 0;
        if (dateFilter === "week" && assetTime < oneWeekAgo) return false;
        if (dateFilter === "month" && assetTime < oneMonthAgo) return false;
        if (dateFilter === "older" && assetTime >= oneMonthAgo) return false;
      }

      if (!q) return true;
      return [asset.label, asset.postTitle, asset.category || "Uncategorized", asset.fileName, asset.typeLabel]
        .join(" ").toLowerCase().includes(q);
    });
  }, [activeType, activeFolder, dateFilter, initialMedia.items, searchQuery]);
  ...
  const folderOptions = useMemo(() => {
    const options = [{ value: "all", label: `All Folders (${initialMedia.items.length})` }];
    uniqueFolders.forEach((folder) => {
      const count = initialMedia.items.filter((asset) => (asset.category || "Uncategorized") === folder).length;
      options.push({ value: folder, label: `${folder} (${count})` });
    });
    return options;
  }, [uniqueFolders, initialMedia.items]);

  const dateOptions = useMemo(() => [
    { value: "all", label: `Any Date (${filterTotals.date.all})` },
    { value: "week", label: `Last 7 Days (${filterTotals.date.week})` },
    { value: "month", label: `Last 30 Days (${filterTotals.date.month})` },
    { value: "older", label: `Older (${filterTotals.date.older})` },
  ], [filterTotals.date]);
          ...
          <div className={styles.mainWrapper}>
                    ...
                    <div className={styles.profileDropdown}>
                      ...
                      <div className={styles.profileDropdownLinks}>
                        ...
                        <Link href="/" className={styles.profileDropdownLink} onClick={() => setIsProfileOpen(false)}>
                          <i className="fas fa-home"></i><span>View Website</span>
                        </Link>
                      </div>
            ...
            <main className={styles.content}>
              ...
              <div className={styles.mlToolbar}>
                ...
                {/* Type dropdown */}
                <div className={styles.mlDropWrap} style={{ width: "180px" }}>
                  <DashboardSelect
                ...
                <div className={styles.mlDropWrap} style={{ width: "180px" }}>
                  <DashboardSelect
                    inputId="media-folder-filter"
                    value={folderOptions.find((option) => option.value === activeFolder) || null}
                    onChange={(option) => {
                      setActiveFolder(option?.value || "all");
                      setCurrentPage(1);
                    }}
                ...
                <div className={styles.mlDropWrap} style={{ width: "180px" }}>
                  <DashboardSelect
                    inputId="media-date-filter"
                    value={dateOptions.find((option) => option.value === dateFilter) || null}
                    onChange={(option) => {
