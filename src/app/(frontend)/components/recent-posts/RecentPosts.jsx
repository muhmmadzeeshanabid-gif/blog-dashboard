...
export default function RecentPosts({
  ...
  const hasSticky = posts.some((p) => p.isSticky);
  const hasGallery = posts.some((p) => p.format === "gallery");
  const orderedPosts = (hasSticky && hasGallery)
    ? [
        ...posts.filter((p) => p.isSticky),
        ...posts.filter((p) => p.format === "gallery"),
        ...posts.filter((p) => !p.isSticky && p.format !== "gallery"),
      ]
    : posts;

  useRecentPostsUi(orderedPosts, currentPage);
  const featuredPost = orderedPosts.find((post) => post.id === featuredPostId) ?? orderedPosts[0];
      ...
      <section className="bwp-recent-posts-section bwp-site-section bwp-section-has-header">
        ...
        <div className="bwp-posts" role="main">
          <div className="bwp-posts-wrap">
            {featuredPost && orderedPosts.length > 0 && (
              <RecentPostArticle
                post={featuredPost}
