...
The project migrated to Supabase but Sanity artifacts remain. **No file in the codebase imports or uses `sanity.js`** — it is 100% dead code.

### Files to Clean

| Action | File | Details |
|--------|------|---------|
| DELETE | `src/app/(backend)/lib/sanity.js` | Full Sanity client with GROQ queries — never imported anywhere |
...
| REWRITE | `document/architecture.md` (line 52) | Remove "Sanity CMS" from hybrid storage description — it's Supabase + local JSON only |
...
### `data/users.json` — Reset completely
...
### `data/action-notifications.json` — Empty it
...
### `data/app-settings.json` — Replace Supabase URLs
...
| `data/users.json` | All avatar URLs — replace with local paths (covered in section 2 above) |
| `data/app-settings.json` | Slide image URL — replace with local path (covered in section 2 above) |
...
## 5. Post Seeding — Re-seed Bug on Cold Starts (HIGH)
...
| Issue | Fix |
|-------|-----|
| Tells buyers to set up Sanity (Step 2) — Sanity is not used | Remove entire Sanity section |
...
| Doesn't mention the `contacts` table | The `db_schema.sql` creates it — setup guide should acknowledge all 4 tables |
...
And remove the `isAdminOrin` special case — just let any valid email pass.
...
### Fix:
Role should come from the database, not from email string matching. The `blog_users` table already has a `role` column — use that as the source of truth.
...
| README references "Next.js 16" | `document/README.md` | Verify this is correct (Next.js 16 is very recent) — currently `package.json` shows `"next": "16.2.9"` so it's accurate |
